...
# 创建数据库连接
db_uri = f"mysql+pymysql://{DATABASE_USER}:{quote_plus(DATABASE_PASSWORD)}@{DATABASE_HOST}:{DATABASE_PORT}/{DATABASE_NAME}?charset=utf8mb4"
...
with engine.connect() as conn:
    ...
    for rule in gdpr_rules:
        # 检查规则是否已存在
        check_sql = text("SELECT COUNT(*) FROM audit_rule WHERE rule_code = :code")
        result = conn.execute(check_sql, {"code": rule[0]})
        exists = result.scalar() > 0

        if exists:
            print(f"规则 {rule[0]} 已存在，跳过")
            skipped_count += 1
        else:
            insert_sql = text("""
                INSERT INTO audit_rule
                (rule_code, rule_name, rule_type, rule_description, rule_expression, severity, is_active, remark)
                VALUES (:code, :name, :type, :desc, :expr, :severity, :active, :remark)
            """)
            ...
            print(f"添加规则: {rule[0]} - {rule[1]}")
            inserted_count += 1

    conn.commit()
    print(f"\n导入完成！")
    print(f"新增规则: {inserted_count} 条")
    print(f"跳过规则: {skipped_count} 条")
    print(f"总计处理: {len(gdpr_rules)} 条")
