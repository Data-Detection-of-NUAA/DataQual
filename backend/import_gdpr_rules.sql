-- GDPR审计规则批量导入脚本
-- 使用方法：mysql -u root -p fastapiadmin < import_gdpr_rules.sql

-- Article 5: 数据处理的七大基本原则
INSERT IGNORE INTO audit_rule (rule_code, rule_name, rule_type, rule_description, rule_expression, severity, is_active, remark) VALUES
...
-- 显示导入结果
SELECT '导入完成！共导入规则：' AS message;
SELECT COUNT(*) AS total_rules FROM audit_rule WHERE rule_code LIKE 'GDPR%';
